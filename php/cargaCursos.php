  ...
  // ini_set('display_startup_errors', 0);
          ...
          if (file_exists("bak_" . $archivo)) {
            ...
            while ($a = $objPHPExcel->getActiveSheet()->getCell('A' . $i)->getCalculatedValue()) {
              echo 'i = ' . $i . ' y j = ' . $j . '<br>';
              if ($a =! "") {
                $_DATOS_EXCEL[$j]['rut'] =trim($objPHPExcel->getActiveSheet()->getCell('A' . $i)->getCalculatedValue());
                ...
                if (PHPExcel_Shared_Date::isDateTime($objPHPExcel->getActiveSheet()->getCell('B' . $i))) {
                  ...
                  $_DATOS_EXCEL[$j]['fecha'] = $fechaDateTime->format('Y-m-d');
                }
                $_DATOS_EXCEL[$j+1]['rut'] =trim($objPHPExcel->getActiveSheet()->getCell('A' . $i)->getCalculatedValue());
                ...
                if (PHPExcel_Shared_Date::isDateTime($objPHPExcel->getActiveSheet()->getCell('C' . $i))) {
                  // Convertir el valor de Excel a un objeto DateTime
                  $fechaDateTime = PHPExcel_Shared_Date::ExcelToPHPObject($fechaExcel);
                  $_DATOS_EXCEL[$j+1]['fecha'] = $fechaDateTime->format('Y-m-d');
                }
                $_DATOS_EXCEL[$j+2]['rut'] =trim($objPHPExcel->getActiveSheet()->getCell('A' . $i)->getCalculatedValue());
                $fechaExcel = $objPHPExcel->getActiveSheet()->getCell('D' . $i)->getCalculatedValue();
                if (PHPExcel_Shared_Date::isDateTime($objPHPExcel->getActiveSheet()->getCell('D' . $i))) {
                  // Convertir el valor de Excel a un objeto DateTime
                  $fechaDateTime = PHPExcel_Shared_Date::ExcelToPHPObject($fechaExcel);
                  $_DATOS_EXCEL[$j+2]['fecha'] = $fechaDateTime->format('Y-m-d');
                }
                $_DATOS_EXCEL[$j+3]['rut'] =trim($objPHPExcel->getActiveSheet()->getCell('A' . $i)->getCalculatedValue());
                $fechaExcel = $objPHPExcel->getActiveSheet()->getCell('E' . $i)->getCalculatedValue();
                if (PHPExcel_Shared_Date::isDateTime($objPHPExcel->getActiveSheet()->getCell('E' . $i))) {
                  // Convertir el valor de Excel a un objeto DateTime
                  $fechaDateTime = PHPExcel_Shared_Date::ExcelToPHPObject($fechaExcel);
                  $_DATOS_EXCEL[$j+3]['fecha'] = $fechaDateTime->format('Y-m-d');
                }
                $i ++;
                $j = $j + 4;
              } else {
            ...
            if(cargaCursosFuncionarios1($_DATOS_EXCEL)){
              // $alerta = [
              //   'Alerta' => 'simple',
